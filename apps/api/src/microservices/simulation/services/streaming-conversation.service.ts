...
export class StreamingConversationService {
      ...
      textChunks.subscribe({
        ...
        complete: async () => {
          try {
            ...
            await this.saveTurn(streamSession, session);
  ...
  private async processUserSentence(
    ...
    try {
      ...
      llmStream$.subscribe({
        ...
        error: (error) => {
          ...
          subject.next({
            ...
            data: {
              error: error.message || 'LLM streaming failed',
              stage: 'llm',
              timestamp: Date.now(),
            },
          });
      ...
      subject.next({
        type: StreamEventType.ERROR,
        data: {
          error: error.message || 'Sentence processing failed',
          stage: 'text',
          timestamp: Date.now(),
        },
      });
  ...
  private async synthesizeAndStreamAudio(
    ...
    try {
      ...
      const ttsConfig = await this.getTTSConfig(session);
  ...
  private async getSessionCached(sessionId: string): Promise<any> {
    // Try cache first
    ...
    if (session) {
      ...
      await this.redis.setSessionCache(sessionId, session as any);
    }
  ...
  private async buildSystemPrompt(session: any): Promise<string> {
    ...
    return await buildConversationSystemPrompt({
  ...
  private resolveLLMConfig(session: any): any {
    const sessionConfig = toRecord(session.sessionConfig) as SessionConfig;
  ...
  private async getTTSConfig(session: any): Promise<{
    provider: string;
    voice?: string;
    language?: string;
  }> {
