...
import type { Session } from '@prisma/simulation-client';
...
export class SessionService {
   ...
   */
  async findAll(query: ListSessionsQueryDto): Promise<SessionListResponseDto> {
   ...
   * Delete a session
   */
  async remove(id: string): Promise<DeleteSessionResponseDto> {
  ...
  private mapToResponseDto(session: Session): SessionResponseDto {
    return {
      ...
      orgId: session.orgId,
      userSnapshot: session.userSnapshot as Record<string, any> | undefined,
