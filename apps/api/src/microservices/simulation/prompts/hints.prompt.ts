...
function resolveStrategyGuidance(strategy: HintStrategy): string {
  switch (strategy) {
    case HintStrategy.PROACTIVE:
      return 'Generate hints proactively to guide the conversation forward.';
    case HintStrategy.REACTIVE:
      return 'Generate hints to re-engage the user when they are inactive or stuck.';
    case HintStrategy.CONTEXTUAL:
      return 'Generate hints based on the current conversation context and gaps.';
    default:
