...
export const STT_DISABLED_RETURN: UseSTTReturn = {
  isListening: false,
  transcript: '',
  interimTranscript: '',
  error: null,
...
export function useWebSpeechSTT(options: UseSTTOptions = {}): UseSTTReturn {
  const {
    lang = 'en-US',
    continuous = true,
    interimResults = true,
  ...
  }, [enabled, lang, continuous, interimResults, maxAlternatives, mapSpeechError])

  useEffect(() => {
    if (!enabled || typeof navigator === 'undefined' || !navigator.permissions?.query) {
      return
    }
  ...
  }, [enabled, isSupported, onError, mapSpeechError])
