...
export class RedisHealthIndicator extends HealthIndicator {
   ...
   */
  private async getRedisInfo(): Promise<RedisHealthCheckResult['details'] | undefined> {
    try {
      const infoStr = await this.redisService.info()
      const info = this.parseRedisInfo(infoStr)

      return {
        connectedClients: info.connected_clients ? parseInt(info.connected_clients, 10) : undefined,
        usedMemory: info.used_memory ? parseInt(info.used_memory, 10) : undefined,
        version: info.redis_version,
        uptime: info.uptime_in_seconds ? parseInt(info.uptime_in_seconds, 10) : undefined,
      }
    } catch (error) {
      return undefined
    }
